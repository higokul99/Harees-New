...
if (isset($_GET['wishlist_action'])) {
    ...
    if ($productId > 0 && !empty($tableName)) {
        ...
        try {
            if ($_GET['wishlist_action'] === 'remove') {
...
<?php
    ...
    $query = "SELECT product_id, table_name FROM user_wishlist WHERE user_id = ?";
    ...
    while ($wishlistItem = $result->fetch_assoc()) {
        ...
        if ($product = $productResult->fetch_assoc()) {
            // Add table name to product data for reference
            ...
            <div class="bg-background text-foreground">
                <?php if (!empty($allProducts)): ?>
                    ...
                    <?php foreach ($allProducts as $product): 
                        $wishlistKey = $product['table_name'] . '_' . $product['id'];
                        $isInWishlist = isset($_SESSION['wishlist'][$wishlistKey]);
                    ?>
                    <a href="product-detail?id=<?php echo $product['id']; ?>&product_code=<?php echo $product['product_code']; ?>&table=<?php echo $product['table_name']; ?>" 
                       data-gender="<?php echo htmlspecialchars($product['gender'] ?? ''); ?>" 
                       data-size="<?php echo htmlspecialchars($product['size'] ?? ''); ?>"
                       data-weight="<?php echo htmlspecialchars($product['net_weight'] ?? ''); ?>"
                       data-price="<?php 
                           $product_code = $product['product_code'];
                           $data = displayRate($conn,$product_code,$product['table_name']); 
                           echo ceil($data[0]);
                       ?>">
                        <div class="product-card">
                            ...
                            <button class="wishlist-btn <?php echo $isInWishlist ? 'active' : ''; ?>"
                                ...
                                data-table-name="<?php echo $product['table_name']; ?>"
                            ...
                            switch ($env) {
                                case 'local':
                                    $loc = "ims/internal/";
                                    $imagedetails = $loc . (!empty($product['img1_webp']) ? $product['img1_webp'] : $product['img2']);
                                    break;
                                case 'dev':
                                    $loc = "ims/internal/";
                                    $imagedetails = $loc . (!empty($product['img1_webp']) ? $product['img1_webp'] : $product['img2']);
                            ...
                            <div class="lazy-image" 
                                 ...
                                 data-alt="<?php echo htmlspecialchars($product['name']); ?>">
