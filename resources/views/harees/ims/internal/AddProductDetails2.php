...
<?php
    ...
    if (isset($_GET['product_code']) && isset($_GET['table_name'])) 
    {
        ...
        //---------METAL
        $metal_name='';
        $Fetch_name_query = "SELECT name FROM metals WHERE metal_id = ?";
        ...
        //---------CATEGORY
        $cat_name='';
        $Fetch_name_query = "SELECT name FROM categories WHERE cat_id = ?";
        if ($stmt = $conn->prepare($Fetch_name_query)) {
        ...
        <form method="POST" action="Control-ProductUpdates.php">
            ...
            if ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    $supplierOptions .= "<option value='{$row['supplier_id']}' $selected>{$row['supplier_name']}</option>";
                }
            } else {
