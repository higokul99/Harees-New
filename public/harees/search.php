...
// Extract price filter from search query if present
$priceFilter = null;
$filteredSearchTerms = [];
...
// If we found a price filter, use the remaining terms for text search
...
// Define all possible product tables
$allTables = [
    ...
    '22kgold_product_nose_pins',
    '22kgold_product_chains',
    '22kgold_product_fancy_chain',
    '22kgold_product_second_studs',
    ...
    '18kgold_product_nose_pins',
    '18kgold_product_chains',
    '18kgold_product_fancy_chain',
    '18kgold_product_second_studs',
    ...
    'silver_product_nose_pins',
    'silver_product_chains',
    'silver_product_fancy_chain',
    'silver_product_second_studs',
    ...
    'rosegold_product_nose_pins',
    'rosegold_product_chains',
    'rosegold_product_fancy_chain',
    'rosegold_product_second_studs',
    ...
    '18kdgold_product_nose_pins',
    '18kdgold_product_chains',
    '18kdgold_product_fancy_chain',
    '18kdgold_product_second_studs'
];

// Store search results
$searchResults = [];

// Function to process database results with price filtering
function processResultsWithPriceFilter($result, $conn, $table, $priceFilterApplied, $priceFilter) {
    $filteredResults = [];
    
    while ($row = $result->fetch_assoc()) {
        try {
            // Calculate price for this product
            $priceData = displayRate($conn, $row['product_code'], $table);
            
            if (empty($priceData)) {
                error_log("Price calculation failed for product {$row['product_code']} in table $table");
                continue;
            }
            
            $price = ceil($priceData[0] ?? 0);
            $row['calculated_price'] = $price;
            
            // Skip price filtering if not applied
            if (!$priceFilterApplied) {
                $filteredResults[] = $row;
                continue;
            }
            ...
            // Apply price filter
            $priceMatch = true;
            if (isset($priceFilter['max']) && $price > $priceFilter['max']) {
                $priceMatch = false;
            }
            if (isset($priceFilter['min']) && $price < $priceFilter['min']) {
                $priceMatch = false;
            }
            ...
            error_log("Error processing product {$row['id']}: " . $e->getMessage());
            continue;
        }
    }
...
foreach ($allTables as $table) {
    ...
    // Check if this table matches any product type filters
    $tableMatchesFilter = empty($productTypeFilters);
    if (!$tableMatchesFilter) {
        $tableParts = explode('_', $table);
        $tableProductType = end($tableParts);
        foreach ($productTypeFilters as $filter) {
            if (strpos($tableProductType, $filter) === 0) { // Starts with
                $tableMatchesFilter = true;
                break;
            }
        }
    }

    if (!$tableMatchesFilter) continue;
    
    // Build search query
    $sql = "SELECT id, name, description, product_code, net_weight, metalpurity_id, cat_id, gender, size, is_featured, img1_webp, img2, delist, stock_quantity, '$table' as source_table FROM `$table` WHERE delist != 1 AND stock_quantity > 0";
    
    $conditions = [];
    $params = [];
    
    // Add text search conditions if we have search terms
    if (!empty($searchTerms)) {
        $textConditions = [];
        foreach ($searchTerms as $term) {
            if (strlen($term) < 2) continue;
            $textConditions[] = "(name REGEXP ? OR description REGEXP ? OR product_code LIKE ?)";
            $termRegex = "\\b" . $conn->real_escape_string($term) . "\\b";
            $termLike = "%$term%";
            $params[] = $termRegex;
            $params[] = $termRegex;
            $params[] = $termLike;
        ...
        if (!empty($textConditions)) {
            $sql .= " AND (" . implode(' AND ', $textConditions) . ")";
        }
    }
    ...
    if (!empty($params)) {
        ...
        // No search terms - get all products and filter by price only
...
<div class="w-full">
    <div class="page-container">
        ...
        <div class="filter-sidebar">
            ...
            <div class="filter-section">
                <h2 class="text-blue-900 text-lg font-semibold">Metal Type</h2>
                <div class="filter-option">
                    <input type="checkbox" id="gold22k">
                    <label for="gold22k">22K Gold</label>&nbsp;&nbsp;&nbsp;
                <!-- </div>
                <div class="filter-option"> -->
                    <input type="checkbox" id="gold18k">
                    <label for="gold18k">18K Gold</label>
                </div>
                <div class="filter-option">
                ...
                <!-- </div>
                <div class="filter-option"> -->
                    <input type="checkbox" id="rosegold">
                    <label for="rosegold">Rose Gold</label>
                </div>
                <div class="filter-option">
                    <input type="checkbox" id="diamond">
                    <label for="diamond">Diamond (18K)</label>
                </div>
            </div>
        ...
        <div class="main-content">
            <h1 class="page-title">Search Results for "<?php echo htmlspecialchars($searchQuery); ?>"</h1>

            <?php if ($priceFilterApplied): ?>
            <div class="active-filters mb-4">
                <div class="filter-chip">
                    Price: 
                    <?php 
                    if (isset($priceFilter['min'])) echo '₹' . number_format($priceFilter['min']) . ' - ';
                    if (isset($priceFilter['max'])) echo '₹' . number_format($priceFilter['max']);
                    elseif (isset($priceFilter['min'])) echo ' and above';
                    ?>
            ...
            <?php if (!empty($searchResults)): ?>
                <div class="products-grid" id="productsGrid">
                    ...
                    <a href="product-detail?id=<?php echo $row['id']; ?>&product_code=<?php echo $row['product_code']; ?>&table=<?php echo $table_name; ?>"
                       ...
                       data-price="<?php echo $row['calculated_price']; ?>">
                        <div class="product-card">
                            ...
                            <?php
                            $imagedetails = "ims/internal/" . (!empty($row['img1_webp']) ? $row['img1_webp'] : $row['img2']);
                            ?>
                            
                            <div class="lazy-image" 
                                 data-src="<?php echo $imagedetails; ?>" 
                            ...
                            <div class="product-info">
                                ...
                                <p class="product-price">₹ <?php echo $row['calculated_price']; ?></p>
    ...
    <div class="mobile-filter-bottom-sheet" id="mobileFilterSheet">
        ...
        <div class="filter-section">
            <h2 class="text-blue-900 text-lg font-semibold">Metal Type</h2>
            <div class="filter-option">
                <input type="checkbox" id="m_gold22k">
                <label for="m_gold22k">22K Gold</label>
            </div>
            <div class="filter-option">
                <input type="checkbox" id="m_gold18k">
                <label for="m_gold18k">18K Gold</label>
            </div>
            <div class="filter-option">
                <input type="checkbox" id="m_silver">
                <label for="m_silver">Silver</label>
            </div>
            <div class="filter-option">
                <input type="checkbox" id="m_rosegold">
                <label for="m_rosegold">Rose Gold</label>
            </div>
            <div class="filter-option">
                <input type="checkbox" id="m_diamond">
                <label for="m_diamond">Diamond (18K)</label>
            </div>
        </div>
...
<!-- The JavaScript code remains exactly the same as in your original file -->
<script>
document.addEventListener("DOMContentLoaded", function () {
    ...
    console.log(`🚀 Lazy loading initialized for ${totalImages} images from multiple tables`);
    
    // Filter functionality
    const priceFilters = {
        'price1': { min: 0, max: 5000 },
        'price2': { min: 5000, max: 10000 },
        'price3': { min: 10000, max: 20000 },
        'price4': { min: 20000, max: 30000 },
        'price5': { min: 30000, max: 50000 },
        'price6': { min: 50000, max: 100000 },
    ...
    const weightFilters = {
        ...
        'gm3': { min: 4, max: 8 },
        'gm4': { min: 8, max: 12 },
        'gm5': { min: 12, max: Infinity }
    };

    const metalTypeMapping = {
        'gold22k': '22kgold',
        'gold18k': '18kgold',
        'silver': 'silver',
        'rosegold': 'rosegold',
    ...
    function applyFilters() {
        ...
        document.querySelectorAll('.filter-sidebar input[type="checkbox"]:checked').forEach(checkbox => {
            ...
            if (checkbox.id in metalTypeMapping) {
                activeMetalFilters.push(metalTypeMapping[checkbox.id]);
            }
        });
        ...
        document.querySelectorAll('.mobile-filter-bottom-sheet input[type="checkbox"]:checked').forEach(checkbox => {
            ...
            if (checkbox.id.startsWith('m_') && checkbox.id.substring(2) in metalTypeMapping) {
                activeMetalFilters.push(metalTypeMapping[checkbox.id.substring(2)]);
            }
        });
        ...
        productItems.forEach(item => {
            ...
            const productTable = item.href.match(/table=([^&]+)/)[1];
            const productMetalType = productTable.split('_')[0];
            ...
            let metalMatch = activeMetalFilters.length === 0;
            ...
            // Check metal type filters
            for (const filter of activeMetalFilters) {
                if (productMetalType.toLowerCase() === filter.toLowerCase()) {
                    metalMatch = true;
                    break;
                }
            }
            ...
            if (priceMatch && genderMatch && sizeMatch && weightMatch && metalMatch) {
    ...
    document.querySelectorAll('.wishlist-btn').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            console.log('Wishlist action:', { action, productId, tableName, isInWishlist });
